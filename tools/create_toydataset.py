...
for i in range(N_CLIPS):
    ...
    color = np.random.randint(0,255, size=(3,)).astype(int)
    color = (int(color[0]),int (color[1]), int(color[2]))
    radius = np.random.randint(10,15)
    for j in range(1,n_img+1):
        new_pos = pos_init + v*j
        bg = np.ones((img_size,img_size,3))*255
        bg = cv2.circle(bg, (new_pos[0], new_pos[1]), radius=radius,
    ...
    # print(direction)
    clip = np.vstack(clip)
    cv2.imwrite(f'local_files/toy_dataset/test/{direction}_{i}.jpeg', clip)
